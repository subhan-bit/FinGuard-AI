...
def score_transaction(transaction: dict, threshold: float = 0.5):
    """
    Score a single transaction dict with keys:
    amount, country, merchant_category, timestamp

    Returns fraud_score (0-1 probability), predicted_fraud (bool),
    and the feature DataFrame (X) used, so callers can reuse it
    for SHAP explanations without rebuilding features twice.
    """
